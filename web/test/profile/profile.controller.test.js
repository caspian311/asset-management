(function() {
  ...
  describe('#update', function() {
    ...
    describe('sucessful update', function() {
      beforeEach(function() {
      ...
      it('show success message after getting a successful backend call', function() {
        scope.update();
        httpBackend.flush();

        expect(scope.successMessage).to.eq('Profile successfully updated.');
      });
    });

    describe('failed update', function() {
      beforeEach(function() {
        httpBackend.when('PUT', userUrl(userId)).respond(400, { });
      });

      it('show error message when backend fails to update', function() {

        scope.update();
        httpBackend.flush();

        expect(scope.errorMessage).to.eq('An error occurred trying to update your Profile.');
      });
    });
  });
  });
